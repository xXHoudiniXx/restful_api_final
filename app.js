       ...
       //random reviews by stars 
        ...
        app.get('/review/:from/:to', (req, res) => {
            ...
            db1.collection("reviews").aggregate({
                $unwind: "$review"
            }, { $group: { _id: $_id, AvgStar: { $avg: $review.star_rating } } }).toArray(function(err, result) {
                                                                                    if (err) throw err;
                                                                                        res.send(result);
                                                                                        db.close();
                                                                                    })
        ...
        app.get('/review/helpful/:prodid', (req, res) => {
            ...
            db1.collection("reviews").aggregate({
            $unwind: "$votes"
            }, { $group: { _id: $_id, AvgVote: { $avg: $votes.helpful_votes } } }).toArray(function(err, result) {
                                                                                    if (err) throw err;
                                                                                    res.send(result);
                                                                                    db.close();
                                                                                    })
        ...
        app.get('/review/info/:custid', (req, res) => {
            ...
            db1.collection("reviews").aggregate({
            $unwind: "$product"
            }, { $group: { _id: $_id, AvgCategory: { $avg: $product.category } } }).toArray(function(err, result) {
                                                                                    if (err) throw err;
                                                                                    res.send(result);
                                                                                    db.close();
                                                                                    })
